...
class setpointPublisher : public rclcpp::Node
{
  public:
    ...
    {
      publisher_ = this->create_publisher<sasdfr_interfaces::msg::Point2::SharedPtr>("setpoint", 10);
  ...
  private:
    ...
    const float64 msg_x[5] = {-0.8, 0.2, -0.4, 0, 0.4};
    const float64 msg_y[5] = {0.4, -0.8, 0.2, -0.4, 0};
    int i = 0;
    
    void timer_callback()
    {
      auto message = asdfr_interfaces::msg::Point2::SharedPtr();
      // Rotate between 5 desired angles every publish
      i = ++i % 5;
      message.x = msg_x[i];
      message.y = msg_y[i];
    
      RCLCPP_INFO(this->get_logger(), "Publishing: x='%s', y='%s'", message.x.c_str(), message.y.c_str());
